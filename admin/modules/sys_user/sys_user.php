...
$old_email = AntiHacker::AntiHackRequest('old_email');
...
$sys_user->email=addslashes(strip_tags($email));
$sys_user->old_login = addslashes(strip_tags($old_login));
$sys_user->old_email = addslashes(strip_tags($old_email));
...
switch( $sys_user->task ){
	...
	case 'change_pass':
		...
		if( !$sys_user->change_pass( $login, $pass) ){
