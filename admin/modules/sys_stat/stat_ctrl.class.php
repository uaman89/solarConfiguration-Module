...
*/
  ...
  */
  function __construct($user_id=NULL, $module=NULL)
  {
   ...
   $this->Logon = &check_init('logon','Authorization');
  ...
  function StatShow()
  {
   ...
   $this->Form->WriteLinkPages( $script1.'&fltr='.$this->fltr.'&fltr_dtfrom='.$this->fltr_dtfrom.'&fltr_dtto='.$this->fltr_dtto.'&fltr_user='.$this->fltr_user, $rows, $this->display, $this->start, $this->sort );

   echo '<TR><TD COLSPAN=19><div class="topPanel"><div class="SavePanel">';

   if($this->Logon->user_type==1){
...
<?
   ...
   for( $i = 0; $i < count( $row_arr ); $i++ )
   {
    ...
    echo '<td align="left">';
    echo '<p style="width:300px;">'.$row['page_url'].'</p>';
