...
class SysSpr extends SystemSpr {
    ...
    */
    function __construct($user_id=NULL, $module=NULL, $display=NULL, $sort=NULL, $start=NULL, $width = NULL, $spr=NULL) {
            ...
            if(isset($this->module) && !empty($this->module) && isset($this->spr) && !empty($this->spr)){
                $this->UploadImages = &check_init('UploadImage', 'UploadImage', $this->module.", null, 'uploads/images/".$this->spr."', 'mod_page_file_img'");
                $this->UploadFile = &check_init('UploadClass', 'UploadClass', $this->module.", null, 'uploads/files/".$this->spr."','mod_page_file'");
            }
    ...
    function AddTbl()
    {
       ...
       if($this->usevisible==1){
           // add field cod_pli
           if ( !$this->Rights->IsFieldExist($this->spr, "visible") ) {
               $q = "ALTER TABLE `".$this->spr."` ADD `visible` set('0','1') NOT NULL DEFAULT '1'";
               $res = $this->Rights->db_Query( $q );
               //echo '<br>$q='.$q.' $res='.$res;
               if( !$res )return false;

               $q = "ALTER TABLE `".$this->spr."` ADD INDEX ( `visible` ) ;";
               $res = $this->Rights->db_Query( $q );
               //echo '<br>$q='.$q.' $res='.$res;
               if( !$res )return false;
           }
       ...
       if($this->usehref==1){
           // add field cod_pli
           if ( !$this->Rights->IsFieldExist($this->spr, "href") ) {
               $q = "ALTER TABLE `".$this->spr."` ADD `href` VARCHAR( 255 )";
               $res = $this->Rights->db_Query( $q );
               //echo '<br>$q='.$q.' $res='.$res;
               if( !$res )return false;
           } 
    ...
    function ShowContentHTML($result=NULL)
    {
        ...
        <table border="0" cellpadding="0" cellspacing="1" width="100%">
         <tr>
          ...
          <?}
          if( $this->usehref==1 ){?>
          <Th class="THead"><? $this->Form->LinkTitle($this->script."&sort=href", $this->Msg_text['_FLD_LINK_NAME']);?></Th>
          <?}
          if( $this->usedescr==1 ){?>
          ...
          <?if ($this->usevisible==1) {?>
          <Th class="THead"><?=$this->Msg_text['FLD_VISIBLE']?></Th>
          <?}?>
...
//                  echo $row['cod'].'<br />node'.$row['node'];
                 ...
                 <?
                 if( $this->usevisible==1 ){?>
                 <td align="center" id="visible<?=$row['cod'];?>">
                    <?
                    $this->ShowVisibility($row['cod'], $row['visible']);
                    ?>
                 </td>
                 <?}
    ...
    * @return
    */
    function ShowVisibility($cod, $visible)
    {
        if( $visible == 0 ) { ?><a href="#" onclick="QuickChangeData('visible<?=$cod;?>', 'module=<?=$this->module;?>&spr=<?=$this->spr;?>&task=change_visible&new_visible=1&cod=<?=$cod;?>');return false;"><?=$this->Form->Img( 'http://'.NAME_SERVER.'/admin/images/icons/publish_x.png', $this->Msg_text['TXT_UNVISIBLE'], 'border=0' );?></a><?}
        if( $visible == 1 ) { ?><a href="#" onclick="QuickChangeData('visible<?=$cod;?>', 'module=<?=$this->module;?>&spr=<?=$this->spr;?>&task=change_visible&new_visible=0&cod=<?=$cod;?>');return false;"><?=$this->Form->Img( 'http://'.NAME_SERVER.'/admin/images/icons/tick.png', $this->Msg_text['TXT_VISIBLE'], 'border=0' );?></a><?}
        return;
    ...
    */
    function ChangeVisibleProp($cod, $new_visible=0)
    {
        $q = "UPDATE `".$this->spr."` SET `visible`='".$new_visible."' WHERE `cod`='".$cod."';";
   ...
   function edit()
   {
    ...
    $this->Form->Hidden( 'usehref', $this->usehref );
    ...
    $this->field_type = mysql_field_type($this->Rights->result,1);
     ...
     ?>
     <tr>
      <td width="100"><b><?echo $this->Msg_text['_FLD_CODE']?>:</b></td>
      <td>
       <?
     ...
     <tr>
         <td>
             <b><?=$this->Msg_text['FLD_VISIBLE']?>:</b></b>
         </td>
         <td>
             <div class="EditTable">
             <?
             ...
             <label for="visibleCheck"><?=$this->Msg_text['FLD_VISIBLE']?></label>
             </div>
         </td>
     </tr>
     <?
...
//       echo '<br>mas=<pre>'; print_r($arr_levels);echo '</pre>';
      ...
      <td colspan="2">
       ...
       while( $el = each( $ln_arr ) )
       {
             ...
             $Panel->WriteItemHeader( $lang );
                ...
                if( $this->usehref==1) {
                    echo "\n <tr>";
                    echo "\n <td><b>".$this->Msg_text['FLD_REFERANSE']."</b>";
                    echo "\n <td>";
                    if( $this->id!=NULL ) $this->Err!=NULL ? $href=$this->href[$lang_id] : $href = $row['href'];
                    else $href=$this->href[$lang_id];
                    $this->Form->TextBox( 'href['.$lang_id.']', stripslashes($href), 40 );
                }
   ...
   function save()
   {
    ...
    while( $el = each( $ln_arr ) )
    {
       ...
       $href = $this->Form->GetRequestTxtData($this->href[$lang_id]);
       ...
       if( $rows>0 )   //--- update
       {
          ...
          if( $this->usevisible==1 ) $q = $q.", `visible`='".$this->visible."'";
          if( $this->usehref==1) $q = $q.", `href`='".$href."'";
...
//        echo '<br> $q='.$q.' $res='.$res.' $this->Rights->result='.$this->Rights->result;
    ...
    if(isset($this->useuploadimages) && $this->useuploadimages==1){
        $res = $this->UploadImages->SaveImages($this->cod);
        //echo '<br>$res='.$res;
    }
    return true;
...
//        echo '<br>q='.$q.' res='.$res.' $tmp_db->result='.$tmp_db->result;
   ...
   function DelImageByCod($cod)
   {
       ...
       $res = $this->db->db_Query( $q, $this->user_id, $this->module );
