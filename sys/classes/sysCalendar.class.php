...
//    }
    ...
    function make_input_field($field_attributes = array(), $cal_options = array(), $id=NULL) {
        if(empty($id)) $id = $this->_gen_id();
        // echo '$this->_field_id($id)='.$this->_field_id($id);
        // echo '$this->_trigger_id($id)='.$this->_trigger_id($id);
        $options = array_merge($cal_options, array('inputField' => $id));
