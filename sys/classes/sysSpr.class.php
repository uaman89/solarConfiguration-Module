...
//         echo '<br>q='.$q.' res='.$res.' $db->result='.$db->result;
   ...
   function DelFromSpr( $spr, $cod, $lang_id = NULL ){
     ...
     function ShowSlider(){
         $tmp_db = new DB();
         $q = "SELECT * FROM `mod_slider` ORDER BY `move`";
         // echo $q;
         $res = $tmp_db->db_Query($q);
         //echo '<br>q='.$q.' res='.$res.' $tmp_db->result='.$tmp_db->result;
         if ( !$res ) return false;
         ...
         $arr = array();
         for($i=0; $i<$rows; $i++)
             $arr[] = $tmp_db->db_FetchAssoc();

         if($rows>0){
             $path = '/images/spr/mod_slider/3/';?>
         <div class="html_carousel">
             <div id="foo2">
                 ...
                 ?>
                 <div class="slide">
                    <?echo $this->ShowImage('mod_slider',$this->lang_id,$row['img'],'size_width=1000',85,NULL,"alt='.$name.'");?>
                     <div>
                         <p><?=strip_tags($row['name']);?></p>
                     </div>
                </div>
                 <?}?>
             </div>
     ...
     function ShowLogo(){
         $tmp_db = new DB();
         $q = "SELECT * FROM `mod_logo` ORDER BY `move`";
         $res = $tmp_db->db_Query($q);
         if ( !$res ) return false;
         if ( !$tmp_db->result ) return false;
             ...
             for ($i=0; $i<$rows; $i++) {
                 ...
                 if($i<4)
                    $c = "line-item";
                     ...
                     $c = "";
                 ?><div class="item-logo <?=$c?>"><?
                 if($row['name']!=""){
                 ?><a href="<?=$row['name']?>" target="_blank"><?
                 }else{
                     ?><a class="no-htr"><?
                 }
                //$spr = NULL, $lang_id = NULL, $img = NULL, $size = NULL, $quality = 85, $wtm = NULL, $parameters = NULL, $ret_img_or_path=null
                 echo $this->ShowImage('mod_logo',$this->lang_id,$row['img'],NULL,100,NULL,"", 1);
     ...
     function ShowSoc(){
         ...
         $tmp_db = new DB();
         $q = "SELECT * FROM `mod_soc` ORDER BY `move`";
         $res = $tmp_db->db_Query($q);
         if ( !$res ) return false;
         if ( !$tmp_db->result ) return false;
         ...
         if($rows>0){
             ...
             for ($i=0; $i<$rows; $i++) {
                 ...
                 ?><?
                     ...
                     //$spr = NULL, $lang_id = NULL, $img = NULL, $size = NULL, $quality = 85, $wtm = NULL, $parameters = NULL, $ret_img_or_path=null
                     echo $this->ShowImage('mod_soc',$this->lang_id,$row['img'],NULL,100,NULL,"", 1);
