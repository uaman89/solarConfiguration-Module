...
//             echo '<br>init_vars1 : $this->session_id='.$this->session_id.' $_SESSION[session_id]='.$_SESSION['session_id'];
        ...
        if (isset($_SERVER['HTTP_X_FORWARDED_FOR']))
            $this->ip2 = $_SERVER['HTTP_X_FORWARDED_FOR'];
        else
            $this->ip2 = NULL;
    ...
    function user_valid($login, $pass, $save_err_to_var = NULL) {
        ...
        if ((!$login)) {
            if ($save_err_to_var == 1)
                $this->Err = $this->Err . $this->Msg->get_msg("_NO_LOGIN") . '<br>';
            else
                $this->Msg->show_msg("_NO_LOGIN");
            return false;
        }

        if ((!$pass)) {
            if ($save_err_to_var == 1)
                $this->Err = $this->Err . $this->Msg->get_msg("_NO_PASS") . '<br>';
            else
                $this->Msg->show_msg('_NO_PASS');
            return false;
        }
        // check how the user logon -  by his login (profile) or by email
        $this->db->db_Query("SELECT * FROM `" . TblSysUser . "` WHERE `login`='" . $login . "'");
        // if no login then search the email
        if (!$this->db->db_GetNumRows()) {
            $q = "SELECT * FROM `" . TblSysUser . "` WHERE `email`='" . $login . "'";
            $this->db->db_Query($q);
            //echo '<br>$q='.$q.' $this->db->result='.$this->db->result;
            //echo '<br>$this->db->db_GetNumRows()='.$this->db->db_GetNumRows();
            if (!$this->db->db_GetNumRows()) {
                if ($save_err_to_var == 1)
                    $this->Err = $this->Err . $this->Msg->get_msg("_LOGIN_INCORRECT") . '<br>';
                else
                    $this->Msg->show_msg('_LOGIN_INCORRECT');
                return false;
            }
            else
        ...
        if ($passwd != $encode_pass) {
            if ($save_err_to_var == 1)
                $this->Err = $this->Err . $this->Msg->show_text("_LOGIN_INCORRECT") . '<br>';
            else
                $this->Msg->show_msg('_LOGIN_INCORRECT');
            return FALSE;
        }
            ...
            if (empty($this->session_id))
                $this->session_id = md5($this->user_id . $this->login . $this->user_type . $this->logintime);
            ...
            if (!$SysUser->set_active($this->user_id))
                return false;
            //set count of Logon
            if (!$SysUser->user_used_counter($this->user_id))
                return false;
...
// End of function SaveSession()
    ...
    // Change Request Nbr:
    // ================================================================================================
    function UpdateSessionAlias($email = NULL) {
        if (!empty($email))
            $this->email = $email;
        $q = "UPDATE `" . TblSysSession . "` SET `email`='" . $this->email . "' WHERE `login`='" . $this->login . "'";
        $this->db->db_Query($q);
        //echo '<br> $q='.$q.' $this->db->result='.$this->db->result;
        if (!$this->db->result)
            return false;
...
// End of function UpdateSessionUserType()
    ...
    function DelOldSession($sec, $front_back = 'back') {
        ...
        if ($front_back == 'back')
            $q = $q . " AND `user_type` IN($str)";
        else
            $q = $q . " AND `user_type` NOT IN($str)";
        $this->db->db_Query($q);
        //echo '<br>$q='.$q.' $this->db->result='.$this->db->result;
        if (!$this->db->result)
            return false;
...
// End of GetLogin_sys_session
    ...
    //
        // Parms : $sid - session_id
    // Returns : $res - login from table sys_session with such session_id
    // Description : get email from table sys_session
    // ================================================================================================
    // Programmer : Igor Trokhymchuk
    // Date : 17.01.2005
    // Reason for change : Creation
