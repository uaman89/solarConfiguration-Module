...
// ================================================================================================
 class SysUser{
       ...
       // Change Request Nbr:
       // ================================================================================================
       function save_sys_user($group, $login, $pass, $enrol_date, $login_multi_use, $last_active_counter, $used_counter, $active, $email)
       {
...
//           echo '<br>$q='.$q.' $res='.$res.' $this->db->result='.$this->db->result;
       ...
       // Change Request Nbr:
       // ================================================================================================
       function update_user($user_id, $group, $email, $enrol_date=NULL, $login_multi_use=NULL)
       {
       ...
       // Date : 12.01.2005
       //
       // Parms : $email - email of user
       // Returns : true - if email exist in system ; else - false
       ...
       // Parms : $email - email of user
       // Returns : true - if email exist in system ; else - false
       // Description : check to unique the email in the table sys_user
       // ================================================================================================
       // Programmer : Igor Trokhymchuk
       // Date : 23.04.2008
       // Reason for change : Creation
       ...
       {
         $q="SELECT * FROM `".TblSysUser."` WHERE `email`='".$email."'";
         $res = $this->db->db_Query($q);
         //echo '<br>q='.$q.' res='.$res.'<br>';
         if (!$this->db->result) return false;
         $rows=$this->db->db_GetNumRows($res);
       ...
       {
         $tmp_db = new DB();
         $q = "select `email` from ".TblSysUser." where `id`='".$user_id."'";
         $res = $tmp_db->db_Query( $q );
         //echo '<br>'.$q.' $res='.$res.' $tmp_db->result='.$tmp_db->result;
         if ( !$res OR !$tmp_db->result) {$tmp_db->db_Close(); return false;}
         $row = $tmp_db->db_FetchAssoc();
       ...
       // ================================================================================================
       // Function : GetUserEmailByUserLogin()
       // Version : 1.0.0
       // Date : 05.05.2008
       //
       // Parms :
       ...
       // Change Request Nbr:
       // ================================================================================================
       function GetUserEmailByUserLogin( $login )
       {
         $tmp_db = new DB();
         $q = "select `email` from ".TblSysUser." where `login`='".$login."'";
         $res = $tmp_db->db_Query( $q );
         //echo '<br>'.$q.' $res='.$res.' $tmp_db->result='.$tmp_db->result;
         if ( !$res OR !$tmp_db->result) {$tmp_db->db_Close(); return false;}
         $row = $tmp_db->db_FetchAssoc();
         ...
         //echo '<br> $login='.$login;
         return $login;
       } //end of fuinction GetUserEmailByUserLogin()
