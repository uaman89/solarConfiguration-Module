            ...
            if ($thumb->img['x_thumb'] >= $src_x OR $thumb->img['y_thumb'] >= $src_y) {
                ...
                if ($wtm == 'txt') {
                    if (defined('WATERMARK_TEXT'))
                        $thumb->txt_watermark = ARTICLE_WATERMARK_TEXT;        // [OPTIONAL] set watermark text [RECOMENDED ONLY WITH GD 2 ]
                    else
                        $thumb->txt_watermark = '';
                ...
                if (!strstr($parameters, 'alt'))
                    $parameters = $parameters . ' alt="' . htmlspecialchars($alt) . '"';
                if (!strstr($parameters, 'title'))
                    $parameters = $parameters . ' title=" ' . htmlspecialchars($title) . ' "';
...
// end of function ConvertDate
    ...
    function Link($cat = NULL, $id = NULL) {
        ...
        if ($id == null) {
            if (!empty($str_cat))
                $link = _LINK . 'articles/' . $str_cat . '/';
            else {
                if ($this->task == 'showa')
                    $link = _LINK . 'articles/last/';
                elseif ($this->task == 'showall')
                    $link = _LINK . 'articles/all/';
                elseif ($this->task == 'arch')
                    $link = _LINK . 'articles/arch/';
                else
                    $link = _LINK . 'articles/';
            }
        }
        else
            $link = _LINK . 'articles/' . $str_cat . '/' . $str_art . '.html';
        return $link;
    }
