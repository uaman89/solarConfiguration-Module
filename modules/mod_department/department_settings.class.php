...
// ================================================================================================
// System : SEOCMS
// Module : department_settings.class.php
// Version : 1.0.0
// Date : 01.07.2010
// Licensed To:
...
//
// Purpose : Class definition for all actions with settings of Department
//
// ================================================================================================
...
//    Returns           : None                                               
//    Description       : Class definition for all actions with managment of Department
// ================================================================================================
...
// ================================================================================================
 class Department_settings extends Department {
    var $set_keywrd;
    var $set_descr;
    var $rss;

    // ================================================================================================
    //    Function          : Department_settings (Constructor)
    //    Version           : 1.0.0
    //    Date              : 01.07.2010
    ...
    function __construct($user_id=NULL, $module=NULL, $display=NULL, $sort=NULL, $start=NULL, $width = NULL) {
            ...
            if (empty($this->Form)) $this->Form = new Form('form_mod_department');
            if (empty($this->Spr)) $this->Spr = new  SysSpr($this->user_id, $this->module);
    } // End of Department_settings Constructor
    ...
    // Description : show setting of Departmentue
    // ================================================================================================
    ...
    $script = $_SERVER['PHP_SELF'].'?module='.$this->module;
     ...
     $tmp_bd= new DB();
      $q1="select * from `".TblModDepartmentSetSprMeta."` where 1";
...
<?}?>
    ...
    if($rows>0)
    {
      $q="update `".TblModDepartmentSet."` set
          `img`='$this->img',
    ...
    else
    {
      ...
      $q="insert into `".TblModDepartmentSet."` values('$this->img', '$this->set_keywrd', '$this->set_descr', '$this->rewrite', '$this->dt', '$this->img_path', '$this->rss')";
      $res = $this->Right->Query( $q, $this->user_id, $this->module );
      //echo '<br>q='.$q.' res='.$res.' $this->Right->result='.$this->Right->result; 
      if( !$this->Right->result) return false;
    }
////////////////////////////////////////////////////////////    
    $q="select * from `".TblModDepartmentSetSprMeta."` where 1";
    $res = $this->Right->Query( $q, $this->user_id, $this->module );
    if( !$this->Right->result ) return false;
    $rows = $this->Right->db_GetNumRows();
    $meta= array();
     ...
     $ln_sys = new SysLang();
            ...
            while( $el = each( $ln_arr ) )
            {
              ...
              if (isset($meta[$lang_id]))
              {
                 ...
                 $q="update `".TblModDepartmentSetSprMeta."` set
...
//                  echo '<br>q='.$q.' res='.$res.' $this->Right->result='.$this->Right->result; 
        ...
        {
          $q="insert into `".TblModDepartmentSetSprMeta."`  set
                  `title`='".$this->title[$lang_id]."',
                  `description`='".$this->description[$lang_id]."',
                  `keywords`='".$this->keywords[$lang_id]."',
                  `lang_id`='$lang_id'";
...
//          echo '<br>q='.$q.' res='.$res.' $this->Right->result='.$this->Right->result; 
 ...
 } //end of class Department_settings
