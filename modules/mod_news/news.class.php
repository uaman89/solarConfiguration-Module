...
class News {
                ...
                if (!in_array($ext, $valid_types)) {
                  ...
                  if (($size) && ($size[0] < $max_image_width) && ($size[1] < $max_image_height)) {
                     ...
                     if ( copy($filename,$uploaddir1) ) {
                         ...
                         $res = $this->db->db_Query( $q );
                         if( !$res OR !$this->db->result ) $this->Err = $this->Err.$this->Msg->show_text('MSG_ERR_SAVE_FILE_TO_DB').' ('.$_FILES['image']['name']["$i"].')<br>';
    ...
    */
    ...
    function Link( $cat = NULL, $id = NULL, $str_news = NULL)
    {
        ...
        if($id==null){
            if(!empty($str_cat)) $link = _LINK.'o-nas/novosti/'.$str_cat.'/';
            else{
                if( $this->task=='showa') $link = _LINK.'o-nas/novosti/last/';
                elseif( $this->task=='showall') $link = _LINK.'o-nas/novosti/all/';
                elseif( $this->task=='arch') $link = _LINK.'o-nas/novosti/arch/';
                else $link = _LINK.'o-nas/novosti/';
            }
        }
        else  {
                $link = _LINK.'o-nas/novosti/'.$str_cat.'/'.$str_news.'.html';
        }
