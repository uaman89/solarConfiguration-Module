...
class META_Export {
	...
	var $available_options = array(
		'market_name', 'encoding', 'file_name',
	);
	...
	var $offers_params = array(
		'id', 'category', 'priority', 'click', 'img', 'vendor', 'type', 'name', 'price', 'priceRUAH', 'priceRUSD', 'priceOUSD', 'currency', 'description',
	);
	...
	var $options = array(
		...
		'file_name'		=> '/xml/meta_price.xml',
	);

	var $fp;

    function META_Export( $options = null ) {
		if ( !is_null($options) ) {
    ...
    function addCategory( $params )
    {
    	...
    	if ( !$init ) {
    		...
    		$code .= '<!DOCTYPE e-shop SYSTEM "http://market.meta.ua/market.dtd">' . "\r\n";
    		$code .= sprintf('<e-shop name="%s">', $this->options['market_name']) . "\r\n";
    		$code .= "\t" . '<categories>' . "\r\n";
    		$this->put_code($code);
    		$init = true;
    	}
		...
		$code = "\t\t";
		$code .= sprintf('<category id="%d" parent="%d"><![CDATA[%s]]></category>', $params['id'], $params['parent'], $params['name']) . "\r\n";
		$this->put_code($code);
    }
	...
	function addOffer( $params )
	{
		...
		$code .= "\t\t\t" . sprintf('<link img="%s" click="%s" />', $params['img'], $params['click']) . "\r\n";
