...
class Bigmir_Export {
    ...
    var $available_options = array(
        'market_name', 'encoding', 'file_name', 'url',
    );
    ...
    var $offers_params = array(
        'id', 'category', 'priority', 'click', 'img', 'vendor', 'type', 'name', 'price', 'currency', 'priceRUAH', 'priceRUSD', 'priceOUSD', 'description',
    );
    ...
    var $options = array(
        ...
        'file_name' => '/xml/bigmir_price.xml',
        'url' => 'http://sitename/',
    );
    var $fp;

    function Bigmir_Export($options = null) {
        if (!is_null($options)) {
    ...
    function addCategory($params) {
        ...
        $code = "\t\t";
        $code .= sprintf('<category id="%d" parentID="%d"><![CDATA[%s]]></category>', $params['id'], $params['parent'], stripslashes($params['name'])) . "\r\n";
        $this->put_code($code);
    }
    ...
    function addOffer($params) {
        ...
        $code .= "\t\t\t" . sprintf('<name><![CDATA[%s]]></name>', stripslashes($params['name'])) . "\r\n";
        $code .= "\t\t\t" . sprintf('<categoryId><![CDATA[%s]]></categoryId>', stripslashes($params['category'])) . "\r\n";
        ...
        if (isset($params['vendor']) && !empty($params['vendor']))
            $code .= "\t\t\t" . sprintf('<vendor><![CDATA[%s]]></vendor>', stripslashes($params['vendor'])) . "\r\n";
        if (isset($params['description']) && !empty($params['description']))
            $code .= "\t\t\t" . sprintf('<description><![CDATA[%s]]></description>', stripslashes($params['description'])) . "\r\n";
        $code .= "\t\t" . '</item>' . "\r\n";
    ...
    function put_code($code, $end = false) {
        ...
        //$code = iconv( 'utf-8' , 'CP1251//TRANSLIT', $code);
