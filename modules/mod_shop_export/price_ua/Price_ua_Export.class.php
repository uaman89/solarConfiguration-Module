...
class Price_ua_Export {
	...
	var $options = array(
		...
		'file_name'		=> '/xml/price_ua_price.xml'
		//'url' 			=> 'http://NAME_SERVER/',
	);

	var $fp;

    function Price_ua_Export( $options = null ) {
		if ( !is_null($options) ) {
    ...
    function addCategory( $params )
    {
        ...
        if($params['parent']!=0)
            ...
            $code .= sprintf('<category id="%d"><![CDATA[%s]]></category>', $params['id'], $params['name']) . "\r\n";
        $this->put_code($code);
    }
	...
	function addOffer( $params )
	{
		...
		*/
		...
		$code .= "\t\t\t" . sprintf('<categoryId>%s</categoryId>', stripslashes($params['category'])) . "\r\n";
